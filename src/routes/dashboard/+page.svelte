...
{#if data.user}
	<div class="space-y-8">
		...
		<div>
			...
			<p class="text-gray-500 mt-1">Manage your projects and bounties</p>
		</div>
		...
		<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
			<div class="card p-6">
				<div class="flex items-center gap-4">
					...
					<div>
						<p class="text-2xl font-bold text-gray-900">{userProjects.length}</p>
			...
			<div class="card p-6">
				<div class="flex items-center gap-4">
					...
					<div>
						<p class="text-2xl font-bold text-gray-900">{userBounties.length}</p>
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
			...
			<div class="card p-6">
				<div class="flex items-center justify-between mb-4">
					...
					<span class="badge badge-teal">{userProjects.length}</span>
				</div>
						...
						{#each userProjects as project}
							<a href="/project/{project.id}" class="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 transition">
								<div class="flex items-center gap-3">
									<div class="w-10 h-10 rounded-lg bg-gradient-to-br from-teal-500 to-cyan-600 flex items-center justify-center text-white font-semibold">
					...
					<div class="space-y-3">
						{#each userBounties as bounty}
							<a href="/project/{bounty.projectId}/bounty/{bounty.id}" class="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 transition">
								...
								<span class="font-semibold text-teal-600">${(bounty.amount / 100).toFixed(2)}</span>
							</a>
						{/each}
					</div>
				{/if}
...
{:else}
	<div class="card p-12 text-center">
		...
		<h2 class="text-2xl font-bold text-gray-900 mb-2">Welcome to OpenLink</h2>
		<p class="text-gray-500 mb-6 max-w-md mx-auto">Sign in to access your dashboard, manage your projects, and track your bounties.</p>
